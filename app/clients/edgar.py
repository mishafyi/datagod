...
async def search_filings(query: str, forms: str = "", startdt: str = "",
                         enddt: str = "", from_: int = 0) -> UpstreamJSON:
    """Full-text search inside filing documents — a thin pass-through to the SEC's
    EFTS endpoint. The SEC returns a FIXED 100 hits per page and ignores any page-size
    request, so paginate with `from_` in steps of 100 (0, 100, 200 … up to ~9900; the
    SEC's result window caps at 10,000). `forms` filters by type; `startdt`/`enddt`
    (YYYY-MM-DD) scope to a filing-date range (EFTS covers 2001+). Returns the SEC
    response unchanged: hits.total, hits.hits (≤100), aggregations."""
    params: dict = {"q": query, "from": from_}
    if forms:
        params["forms"] = forms
    if startdt:
